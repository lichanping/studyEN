...
6. 显示反馈后，用户点击“下一题”继续。
7. 用户可随时点击“交卷”；存在未答题时，弹窗提示未答数量，用户可继续答题或确认交卷。
8. 确认交卷或最后一题确认后，进入与整卷批改共用的结果页，展示实际得分、总分、百分比、各题型统计和逐题结果汇总。
...
3. 手机端组卷设置应采用紧凑布局：三种题型各占一行，每行的题量与分值同排；无错误时错误提示区不占高度；两种批改方式同排，避免设置页因卡片纵向堆叠过长。
4. 状态标识不能只依赖颜色，需同时包含文字或图标。
5. 按钮和选项应支持键盘聚焦，表单控件应有关联标签。
6. 题库 JSON 加载失败时展示明确错误和重试入口，不进入空白试卷。
7. 全部功能在浏览器本地运行；MVP 不新增后端服务、数据库或 Netlify Function。
8. 页面资源使用相对路径，确保 Netlify 根目录静态发布后可直接访问；CSS 或 JavaScript 内容更新时同步更新页面引用的版本参数，避免浏览器继续使用旧资源。
9. 题库 JSON 使用浏览器 HTTP 缓存降低重复下载；采用版本化或内容哈希文件名并设置长期缓存，发布新题库时通过新 URL 完成缓存失效。
10. 缓存命中时直接使用缓存题库；首次加载或缓存不可用时正常从网络获取。MVP 不承诺完全离线可用。
...
| 手机设置页高度 | 在 320px 至 430px 视口查看组卷设置 | 三种题型各占紧凑一行，题量与分值同排，空错误区不占高度，两种批改方式同排 |
