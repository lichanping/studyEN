...
1. 单选题数量输入框，默认值 20，并展示从 JSON 动态得到的库存上限。
2. 多选题数量输入框，默认值 10，并展示从 JSON 动态得到的库存上限。
3. 判断题数量输入框，默认值 20，并展示从 JSON 动态得到的库存上限。
4. 答题模式选择：
...
10. 支持“重新设置”：返回组卷设置页。
11. MVP 成绩和错题仅保留在当前页面会话中，刷新或关闭页面后不保存。
...
4. 默认组卷数量为单选题 20、多选题 10、判断题 20，用户可修改。
