...
7. 邮件文案需带上学员名、异常说明摘要、订阅关注字段，以及本轮最新的 `30分钟剩余 / 60分钟剩余 / 陪练服务时长剩余` 快照和检测结果（仍不足 / 已恢复）。
8. 邮件正文不再展示“检查范围”“基础排课 X 条 + 临时加课 Y 条”这类排课上下文；异常学生订阅邮件只保留与当前额度订阅直接相关的信息。
9. 两类订阅提醒邮件正文都需展示当前是第几次轮询：第 1/2 次分别显示 `轮询次数：1`、`轮询次数：2`，最后一次显示 `轮询次数：3(last)`。
...
6. AC-24：未排课提醒邮件与异常学生提醒邮件均展示当前轮询次数；最后一次提醒明确标记为 `3(last)`。