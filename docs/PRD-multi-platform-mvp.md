...
6. index.html（正式课入口页）
- 增加平台选择控件。
- 当平台为“百分缔”时，支持上传单词表图片并自动提取文本到“新学单词（英文+中文）”输入区；识别范围以“抗遗忘复习计划”上方的全部词表区域为准，按图片里实际非空单元格数量导入，减少手动录入。
...
9. AC-09：在 `index.html` 切换到“百分缔”后，支持上传单词表图片并自动回填到“新学单词（英文+中文）”输入区；识别范围覆盖“抗遗忘复习计划”上方的全部词表区域，且按图片实际非空词格数量导入，不写死为 26 词样本。
