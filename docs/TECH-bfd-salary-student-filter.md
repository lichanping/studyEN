...
工资资格仍只由有效工资事件决定。输出阶段读取 `hidden-students-v1`：标准姓名命中黑名单时，在文本摘要和 CSV 的正课明细、额外复习明细、学生总计中统一显示 `姓名（已退学·当月有工资）`。展示后缀不参与聚合键、金额计算或数据持久化。
...
4. 黑名单中的有效工资学生保留真实姓名，并追加 `（已退学·当月有工资）`。