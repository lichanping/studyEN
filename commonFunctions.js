...
export function parseLocalDateYmd(dateLike) {
...
export function handleManagementGroupTemplateClick() {
    ...
    if (timeDifference > 0 && timeDifference <= 30) {
        ...
        outputMessage = `【抗遗忘温馨提醒-${hour}:${minute}】<br><br>👍 ${(window.APP_MEETING_CONFIG?.getCurrentAntiForgettingTencentMeetingTag?.() || window.APP_MEETING_CONFIG.antiForgettingTencentMeetingTag || window.APP_MEETING_CONFIG?.getCurrentTencentMeetingTag?.() || window.APP_MEETING_CONFIG.tencentMeetingTag)}<br><br>⏰ 请准时参加会议，如有特殊情况无法参加，请提前告知！<br><br>🔔 若未按约定时间参加复习，请自主复习并打卡至学习群📱<br><br>🔒 仅在当天有收到提醒通知才需入会。<br><br>📢 请在抗遗忘时及时反馈是否需要取消下次安排的正课，以便妥善安排教学计划，老师为你骄傲🌹`;
    }
